...
public interface OrderMapper {
  ...
  @Insert("INSERT INTO tb_order (user_id, seat_id, appointment_time) VALUES (#{userId}, #{seatId}, #{appointmentTime})")
  ...
  @Select("SELECT * FROM tb_order WHERE appointment_time = #{appointmentTime} AND seat_id = #{seatId}")
  @Results({
      @Result(property = "orderId", column = "order_id"),
      @Result(property = "userId", column = "user_id"),
      @Result(property = "seatId", column = "seat_id"),
  ...
  Order[] getOrdersBySeatAndAppointmentTime(int seatId, Timestamp appointmentTime);
  ...
  @Update("UPDATE tb_order SET order_status = 2 WHERE appointment_time + INTERVAL '1' HOUR < NOW() AND order_status = 0")
